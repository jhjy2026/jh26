...
class QQWry {
    ...
    private function parseRecord($record) {
        ...
        // 转换中文编码
        $this->country = mb_convert_encoding($this->country, 'GB2312', 'UTF-8');
        echo "国家：{$result['country']}\n";
        $this->province = mb_convert_encoding($this->province, 'GB2312', 'UTF-8');
        $this->city = mb_convert_encoding($this->city, 'GB2312', 'UTF-8');
        $this->isp = mb_convert_encoding($this->isp, 'GB2312', 'UTF-8');
    }
...
try {
    $qqwry = new QQWry();
    $result = $qqwry->getIpInfo('202.96.128.0');
    $qq='AAAA 实验输出';
